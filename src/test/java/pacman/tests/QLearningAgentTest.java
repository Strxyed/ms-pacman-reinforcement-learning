...
class QLearningAgentTest {
    ...
    void qLearningImprovesWithMoreTraining() {
        QLearningAgent agent = (QLearningAgent) TestProgressTracker.getObject("QL_AGENT_BASELINE");
        if (agent == null) {
    ...
    private void trainAdditionalEpisodes(QLearningAgent agent, int episodesPerCall) {
        ...
        try {
            Field field = QLearningAgent.class.getDeclaredField("episodes");
    ...
    private Map<Object, Double> snapshotQTable(QLearningAgent agent) {
        try {
            Field field = QLearningAgent.class.getDeclaredField("qTable");
            field.setAccessible(true);
    ...
    private void restoreQTable(QLearningAgent agent, Map<Object, Double> snapshot) {
        try {
            Field field = QLearningAgent.class.getDeclaredField("qTable");
            field.setAccessible(true);
            @SuppressWarnings("unchecked")
